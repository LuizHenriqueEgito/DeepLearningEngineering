...
class LMPipeline:
    ...
    def decoder_standard_generation(
        ...
        # tokenized = [self.sos_token] + self.tokenizer.tokenize_text(input_text).tolist()  # padding=False, truncation=False
        tokenized = self.tokenizer.tokenize_text(input_text).tolist()
        ...
        for _ in range(max_tokens):
            ...
            for current_text in generated_texts:
                ...
                chosen, logprob = self.new_decoder_sample(
                    current_text.get_tokens_for_decoder(
    ...
    def decoder_nucleus_generation(
        ...
        for i in range(max_tokens):
            ...
            last = self.model.lm_head(pre_logits[:, -1, :]).reshape((-1,))
